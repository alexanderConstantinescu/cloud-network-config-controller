...
func (c *CloudNetworkConfigController) processNextWorkItem() bool {
	...
	err := func(obj interface{}) error {
		...
		if key, ok = obj.(string); !ok {
			...
			utilruntime.HandleError(fmt.Errorf("expected string in %s workqueue but got %#v", c.controllerKey, obj))
			return nil
		}
...
func (c *CloudNetworkConfigController) Enqueue(obj interface{}) {
	...
	if !ok {
		utilruntime.HandleError(fmt.Errorf("error decoding object, object not of correct type: %#v", obj))
		return
	}
	obj = tombstone.Obj
	if c.controllerType != reflect.TypeOf(obj) {
		utilruntime.HandleError(fmt.Errorf("error decoding object tombstone, object not of correct type: %#v", obj))
		return
	}
	key, err := cache.MetaNamespaceKeyFunc(obj)
	if err != nil {
